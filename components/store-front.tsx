    ...
    <div className="w-full">
      <Tabs defaultValue={categories[0].id} className="w-full">
        <div className="flex justify-center mb-10">
          <TabsList className="theme-pill h-12 p-1 rounded-full">
            {categories.map((cat) => (
              <TabsTrigger 
                key={cat.id} 
                value={cat.id}
                className="theme-tab-trigger rounded-full px-6 py-2 text-sm font-medium transition-all"
              >
                {cat.name}
              </TabsTrigger>
            ))}
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {cat.products.map((product) => (
                  <Card 
                    key={product.id} 
                    className={cn(
                      "theme-card group relative overflow-hidden transition-all duration-300",
                      product.stock > 0 
                        ? "hover:-translate-y-1 theme-card-hover" 
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-background/95 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 flex flex-col justify-center items-center p-6 text-center">
                       <h4 className="theme-emphasis-title font-bold text-lg mb-2">{product.name}</h4>
                    ...
                    <CardContent>
                       <div className="flex items-center justify-between">
                         <div className="theme-price flex items-baseline gap-1">
      ...
      <Dialog open={isBuyOpen} onOpenChange={setIsBuyOpen}>
        <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto md:overflow-hidden p-0 gap-0">
          <div className="grid grid-cols-1 md:grid-cols-5 md:h-full">
            ...
            <div className="order-2 md:order-1 md:col-span-3 p-6 flex flex-col md:h-full md:overflow-y-auto bg-background">
              <DialogHeader className="mb-4">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <Badge variant="secondary" className="theme-pill theme-price border-primary/20">
              ...
              <div className="theme-prose flex-1 prose prose-sm dark:prose-invert max-w-none pr-2">
            ...
            <div className="theme-sidebar order-1 md:order-2 md:col-span-2 border-b md:border-b-0 md:border-l p-6 flex flex-col gap-6 md:h-full md:overflow-y-auto">
              <div className="space-y-4">
                ...
                <div className="grid gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    {appliedCoupon ? (
                      <Button variant="outline" size="icon" onClick={() => { setAppliedCoupon(null); setCouponCode(""); }} className="theme-outline-button shrink-0">
                        <X className="h-4 w-4" />
                      </Button>
                    ) : (
                      <Button variant="secondary" onClick={handleApplyCoupon} disabled={isValidatingCoupon || !couponCode.trim()} className="theme-outline-button shrink-0">
              ...
              <div className="mt-auto pt-6 border-t space-y-4">
                ...
                <Button size="lg" className="theme-cta-button w-full font-bold text-lg h-12" onClick={handlePurchase} disabled={loading}>
