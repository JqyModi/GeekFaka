...
- 后台「系统设置」->「货源测试」可开启 `上游采购 dry-run`
- 开启后可用真实支付网关验证支付回调、订单状态、供应商履约和卡密展示
- dry-run 会生成 `dryrun-...` 模拟卡密，不会真实请求上游采购，也不会扣减上游余额
- 测试完成后必须关闭该开关，并确认环境变量 `SUPPLIER_PURCHASE_DRY_RUN` 不是 `true`
